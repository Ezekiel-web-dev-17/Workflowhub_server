...
const userToken = () => `Bearer ${generateAccessToken({ userId: "user-cuid-001", email: "alice@example.com" })}`;
/** JWT for an admin user. */
const adminToken = () => `Bearer ${generateAccessToken({ userId: "admin-cuid-001", email: "admin@example.com", role: "ADMIN" })}`;
...
beforeEach(() => {
    ...
    mockReview.aggregate.mockResolvedValue({ _avg: { rating: 4.5 }, _count: { rating: 10 } });
});
...
describe("GET /api/v1/tools", () => {
    ...
    it("accepts valid query params: search, sortBy, order, page, limit (200)", async () => {
        ...
        const res = await request(app)
            .get("/api/v1/tools")
            .query({ search: "VSCode", sortBy: "rating", order: "desc", page: 1, limit: 10 });
...
describe("PATCH /api/v1/tools/:id", () => {
    it("updates a tool's description when requester is admin (200)", async () => {
        ...
        mockTools.update.mockResolvedValue({ ...TOOL, description: "Updated description" });
...
describe("PATCH /api/v1/tools/:id/reviews", () => {
    it("lets a user update their own review (200)", async () => {
        ...
        mockReview.update.mockResolvedValue({ ...REVIEW, content: "Updated opinion" });
